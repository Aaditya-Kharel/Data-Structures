...
//#include "queue.tpp" // Include the implementation
...
//#include "node2.h"   // Node template class
    ...
    // Library facilities used: cassert, node2.h
    {
        ...
        rear_ptr->set_link(front_ptr);      //Makes the rear node point to new head node
        --count;
    }
    ...
    // Library facilities used: node2.h
    {
        ...
        {   // Insert an entry that is not the first.
            ...
            //rear_ptr->setlink(front_ptr);       //making rear_ptr link_field point to front_ptr
        }
        ++count;
    }
    ...
    void queue<Item>::print(){
        ...
        if(cursor->data()==rear_ptr->data())
            break;
        //std::cout<<rear_ptr->data()<<std::endl;
        //std::cout<<rear_ptr->link()->data()<<std::endl;
    }
    }
#endif
