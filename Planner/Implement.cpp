...
using namespace std;
using namespace aaditya_kharel_hw3;
...
chores::chores() {
	...
	if (used+1 < capacity) {
		choresPtr = new string[capacity];
	}

	else if (used >= capacity) {
		choresPtr = new string[capacity * 2];
	}

	while (getline(inFile, str)) {	//takes input from file,  
		choresPtr[used] = str;		//stores in str &
...
void chores::deleteitem() {
	...
	int delete_position;
	cin >> delete_position;
	choresPtr[delete_position - 1] = choresPtr[used - 1];
	used = used - 1;
